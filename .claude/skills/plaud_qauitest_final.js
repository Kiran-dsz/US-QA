...
async function runProductionTest() {
  ...
  try {
    ...
    // Set to full screen resolution (macOS typical full screen)
    ...
    // Click "Personal use" option on product routing page if present
    console.log('[TEST 3/8] Clicking "Personal use" option...');
    const personalUseClicked = await page.evaluate(() => {
      const allElements = document.querySelectorAll('*');
      for (let el of allElements) {
        const text = (el.textContent || '').trim();
        if (text === 'Personal use' || (el.textContent.includes('Personal use') && el.textContent.length < 100)) {
          // Find the clickable container
          let clickable = el;
          let depth = 0;
          while (clickable && depth < 5 && !['BUTTON', 'A'].includes(clickable.tagName)) {
            clickable = clickable.parentElement;
            depth++;
          }
          if (clickable && clickable.offsetHeight > 0) {
            clickable.click();
            console.log('Personal use option clicked');
            return true;
          }
        }
      }
    ...
    if (personalUseClicked) {
      console.log('✓ Personal use selected');
      await delay(3000);
    } else {
      console.log('⚠ Personal use option not found - page may have progressed already');
    }

    await delay(1000);

    // TEST 4 & 5: Navigate to Membership Center
    console.log('[TEST 4/8] Navigate to Membership Center...');
    ...
    // Get all available currencies from dropdown dynamically
    console.log('  Getting ALL available currencies...');
    const currencyDropdownOpen = await page.evaluate(() => {
      const elements = Array.from(document.querySelectorAll('button, select, [role="button"], [role="combobox"]'));
      for (let el of elements) {
        const text = el.textContent.toLowerCase();
        if (text.includes('prices') && text.includes('usd')) {
    ...
    // Known currencies available in the dropdown (detected from previous tests)
    ...
    console.log(`  Currencies to test: ${currencyOptions.map(c => c.code).join(', ')}`);

    // Test each currency and record prices
    const currencyTests = [];

    // First, get and record USD prices (base)
    const usdPrices = await page.evaluate(() => {
      const text = document.body.innerText;
      const prices = text.match(/\$[\d,]+(?:\.\d{2})?/g) || [];
      return prices.slice(0, 3);
    });
    currencyTests.push({ currency: 'USD', symbol: '$', prices: usdPrices, selected: true });
    console.log(`  USD $: ${usdPrices.join(', ')}`);

    // Test each additional currency
    for (const currencyOpt of currencyOptions) {
      if (currencyOpt.code === 'USD') continue;

      console.log(`  Testing ${currencyOpt.code}...`);

      // Open dropdown
      await page.evaluate(() => {
        const buttons = Array.from(document.querySelectorAll('button, [role="button"]'));
        for (let btn of buttons) {
          if (btn.textContent.toLowerCase().includes('prices') || btn.textContent.toLowerCase().includes('usd')) {
            btn.click();
            break;
          }
        }
      ...
      const selected = await page.evaluate((code) => {
        ...
        for (let el of elements) {
          const text = (el.textContent || '').trim();
          if (text.startsWith(code) && text.length < 20) {
            let clickable = el;
            while (clickable && !['BUTTON', 'A', 'LI', 'OPTION', 'DIV'].includes(clickable.tagName)) {
              clickable = clickable.parentElement;
            }
            if (clickable && clickable.offsetHeight > 0 && clickable.offsetWidth > 0) {
              clickable.click();
              return true;
            }
          }
      ...
      if (selected) {
        console.log(`    ✓ ${currencyOpt.code} selected`);
        await delay(2500);

        // Extract prices based on currency
        const prices = await page.evaluate((symbol) => {
          const text = document.body.innerText;
          let pattern;

          if (symbol === 'HK$') pattern = /HK\$[\d,]+(?:\.\d{2})?/g;
          else if (symbol === 'NT$') pattern = /NT\$[\d,]+(?:\.\d{2})?/g;
          else if (symbol === '$') pattern = /\$[\d,]+(?:\.\d{2})?/g;
          else if (symbol === '€') pattern = /€[\d,]+(?:\.\d{2})?/g;
          else if (symbol === '£') pattern = /£[\d,]+(?:\.\d{2})?/g;
          else if (symbol === '¥') pattern = /¥[\d,]+(?:\.\d{2})?/g;
          else pattern = new RegExp(symbol.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '[\\d,]+', 'g');

          const found = text.match(pattern) || [];
          return found.slice(0, 3);
        }, currencyOpt.symbol);

        currencyTests.push({ currency: currencyOpt.code, symbol: currencyOpt.symbol, prices: prices, selected: true });
        console.log(`      Prices: ${prices.join(', ') || 'None found'}`);
      } else {
        console.log(`    ⚠ ${currencyOpt.code} not found/clickable`);
        currencyTests.push({ currency: currencyOpt.code, symbol: currencyOpt.symbol, prices: [], selected: false });
      }
    }

    // SCREENSHOT 3: After currency changes
    console.log('📸 Taking screenshot: After currency testing');
    ...
    // Verify at least EUR works
    const eurPricesValid = currencyTests.find(c => c.currency === 'EUR')?.prices?.length > 0;
    const pricesChanged = eurPricesValid && currencyTests[0]?.prices?.[0] !== initialPrices[0];
    const status = eurPricesValid ? 'PASS' : 'PARTIAL';

    console.log(`${pricesChanged ? '✓ PASS' : '⚠ PARTIAL'} - Currency conversions ${pricesChanged ? 'working successfully' : 'partially working'}`);
